...
namespace Core.Scripts.Utils.CustomCollections
{
    ...
    /// </summary>
// [JsonConverter(typeof(FixedUnorderedListJsonConverter<>))]
    public sealed class FixedUnorderedList<T>
    {
        [JsonIgnore]
        private readonly T[] _items;
        private int _count;

        public FixedUnorderedList(int capacity)
        {
            ...
            set
            {
                ...
                if (value.Length > _items.Length)
                    throw new InvalidOperationException(
                        $"Deserialized item count {value.Length} exceeds FixedUnorderedList capacity {_items.Length}");
        ...
        public bool Contains(ushort targetId)
        {
            for (int i = 0; i < Count; i++)
            {
                if(RawArray[i].Equals(targetId))
                {
                    return true;
                }
            }
