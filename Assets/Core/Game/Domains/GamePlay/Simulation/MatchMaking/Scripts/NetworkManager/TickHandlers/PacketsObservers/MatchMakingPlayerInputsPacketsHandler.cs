...
namespace Core.Game.Domains.GamePlay.Simulation.MatchMaking.Scripts.NetworkManager.TickHandlers.PacketsObservers
{
    public class MatchMakingPlayerInputsPacketsHandler : IPlayerInputsPacketsHandler
    {
        ...
        private void UpdatePlayerDirection(MatchMakingPlayerInputPacketC2S playerInputPacket, MatchMakingPlayerStateS2C playerModel)
        {
            ...
            var rotationAngle =
                (GetInputForPlayer(playerInputPacket, playerModel.Id).IsMoveLeftInputPressed.ToInt() -
                 GetInputForPlayer(playerInputPacket, playerModel.Id).IsMoveRightInputPressed.ToInt()) * rotationDelta;
            ...
            if (GetInputForPlayer(playerInputPacket, playerModel.Id).IsMoveForwardInputPressed)
            {
...
var indexOfEarliestInput = GetIndexOfEarliestInput(playerInputs);
        ...
        private void OnPlayerInputReceived(MatchMakingPlayerInputPacketC2S playerInputPacket, NetPeer peer)
        {
            ...
            for(int _i=0; _i<playerInputPacket.Inputs.Length; _i++)
            {
                ...
                input = new MatchMakingPlayerInputPacketC2S { Tick = playerInputPacket.Tick, HeighestProcessedTickFromServer = playerInputPacket.HeighestProcessedTickFromServer, Inputs = new MatchMakingPlayerInputData[] { inputData } };
            }
        }
        ...
        private MatchMakingPlayerInputData GetInputForPlayer(MatchMakingPlayerInputPacketC2S packet, ushort playerId)
        {
