...
| 10 | **Chicken** (`ChickenEgg`, **static + sensor**) | Lays eggs that break on player contact | **Decision required — recommend: gate crushes eggs.** The egg is `Static` and the gate is `Dynamic`, so Box2D **does** pair them (unlike the egg↔mole static-static case that needed `TryBreakChickenEggsOnMolesCommand`). Add `ScoreGate` to the egg's mask and a `HandleChickenEggScoreGateCollision` that breaks the egg with no impulse on the gate. Alternative: no interaction at all (leave masks untouched) — cheaper, but eggs visibly clipping through a rolling gate looks broken. | `PhysicsCollisionFilters` `ChickenEgg` case + `ProcessCachedCollisionsCommand` |
...
| 15 | **FishingRod** (`FishingRodTip`, **sensor**) | Tip flies out; on `Wall` it retracts, on an enemy it catches and reels them in | **Decision required — recommend: treat the gate as a wall.** Add `ScoreGate` to `HandleFishingRodTipWallCollision` so the tip retracts instead of passing through, plus a small `PushScoreGateCommand`. The richer option — *catching* the gate and reeling it in like an enemy — needs `FishingRodCaughtEnemyType` to gain a non-player case, the reel logic to drive a Box2D body instead of a `PlayerStateS2C`, and new client art for a gate-on-a-line; that is its own feature, not this one. Flag it as a follow-up. | `ProcessCachedCollisionsCommand.HandleFishingRodTipWallCollision` |
...
- [ ] **Chicken egg vs gate decision made** (recommend: gate crushes the egg — they *do* pair, since the egg is static and the gate is dynamic).
- [ ] **Fishing rod "catch the gate" deferred**, not half-built — the wall-like retract is this feature's scope.
...
4. **Walk the whole §6.4.1 matrix with `ShouldChooseRandomTalentsForPlayer` off and each talent forced in turn:**
   ...
   - **GrapplingHook** → anchors on the gate and reels you in, and the anchor tracks the gate as it moves. **FishingRod** → tip retracts off it. **Soul** → ghost stops on it.
   - **Chicken** → an egg under the gate breaks as the gate rolls over it (per the §6.4.1 #10 decision).
